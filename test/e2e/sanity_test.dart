...
void main() {
  ...
  test('Sonnet 4.6 via ClaudeCodeOpenAIClient (OAuth)', () async {
    ...
    print('Usage: promptTokens=${response.usage?.promptTokens}, '
  ...
  test('Sonnet 4.6 with custom tools via OAuth', () async {
    ...
    print('Tool call response: ${msg.content ?? "tool_calls: ${msg.toolCalls?.map((t) => "${t.function.name}(${t.function.arguments})").join(", ")}"}');
  ...
  test('Haiku still works via OAuth', () async {
    final client = ClaudeCodeOpenAIClient(credentials: creds);
